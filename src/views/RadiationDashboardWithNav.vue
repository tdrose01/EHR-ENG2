    ...
    <div class="bg-gray-900 border-b border-gray-700 p-6">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          ...
          <button
            @click="goToNavmed"
            class="bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded-lg transition-colors inline-flex items-center"
            type="button"
          >
        ...
        <div class="p-4 sm:p-6 bg-gray-900">
          ...
          <div v-if="activeTab === 'reconciliation'" class="space-y-4">
            <div class="flex justify-between items-center mb-4">
              ...
              <div class="flex space-x-2">
                <select
                  v-model="reconciliationPeriodFilter"
                  class="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white"
                >
                  <option value="">All Periods</option>
                  <option
                    v-for="period in reconciliationPeriods"
                    :key="period.value"
                    :value="period.value"
                  >
                    {{ period.label }}
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full bg-gray-700 rounded-lg">
                ...
                <tbody v-if="filteredReconciliations.length > 0" class="divide-y divide-gray-600">
                  <tr v-for="recon in filteredReconciliations" :key="recon.id" class="hover:bg-gray-600 transition-colors">
                ...
                <tbody v-else>
                  <tr>
                    <td colspan="7" class="px-4 py-8 text-center text-gray-400">
                      ...
                      <div>{{ reconciliationPeriodFilter ? 'No reconciliation data for the selected period' : 'No reconciliation data available' }}</div>
                      <div class="text-sm">{{ reconciliationPeriodFilter ? 'Try a different period or clear the filter.' : (reconciliationsError || 'Loading...') }}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
...
export default {
  ...
  setup() {
    ...
    const buildReconciliationPeriodLabel = (startDate, endDate) => {
      if (!startDate) {
        return ''
      }
    ...
    const reconciliationPeriods = computed(() => {
      const labels = reconciliations.value
        .map(record => record.period_label)
        .filter(Boolean)
    ...
    const filteredReconciliations = computed(() => {
      ...
      return reconciliations.value.filter(
        recon => recon.period_label === reconciliationPeriodFilter.value
      )
    })
    ...
    const fetchReconciliations = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json()
          reconciliations.value = data.map(record => ({
            ...record,
            op_hp10_msv: toNumber(record.op_hp10_msv),
            ndc_hp10_msv: toNumber(record.ndc_hp10_msv),
            variance_msv: toNumber(record.variance_msv),
            period_label: buildReconciliationPeriodLabel(record.period_start, record.period_end)
          }))
        } else {
          const errorData = await response.json()
